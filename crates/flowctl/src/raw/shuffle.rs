...
impl Shuffle {
    pub async fn run(&self, ctx: &mut crate::CliContext) -> anyhow::Result<()> {
        ...
        // Start the shuffle server in the background.
        let addr = format!("127.0.0.1:{port}");
        let server_addr = addr.clone();

        let service = shuffle::ShuffleService::with_defaults();
        let server = service.build_tonic_server();

        let listener = tokio::net::TcpListener::bind(&server_addr)
            .await
            .with_context(|| format!("binding to {server_addr}"))?;

        tracing::info!(addr = %server_addr, member_count, "starting shuffle server");
        ...
        let server_handle = tokio::spawn(async move {
            ...
            server
                ...
                .await
                .context("shuffle server error")
        });

        // Give the server a moment to start.
        tokio::time::sleep(std::time::Duration::from_millis(100)).await;

        // Connect as a client and open a Session.
        let client_addr = format!("http://{addr}");
        tracing::info!(addr = %client_addr, member_count, "connecting to shuffle server");

        let mut client = ShuffleClient::connect(client_addr.clone())
            .await
            .context("connecting to shuffle server")?;

        // Create member topology with split key ranges, all pointing to same address.
        let members = build_member_topology(*member_count, &client_addr);
        ...
        // Abort the server (we're done testing).
        server_handle.abort();
...
/// All members point to the same address (for local testing).
fn build_member_topology(count: u32, address: &str) -> Vec<Member> {
    ...
    tracing::info!(
        count,
        "built member topology with key ranges: {:?}",
        members
            .iter()
            .map(|m| {
                let r = m.range.as_ref().unwrap();
                format!("[{:#x}, {:#x}]", r.key_begin, r.key_end)
            })
            .collect::<Vec<_>>()
    );
